...
export default function AdminDashboard() {
  ...
  return (
    <div className="content-area active">
      ...
      {showGateScanner && (
        <section className="card" style={{ border: '2px solid #ff9800', backgroundColor: '#fff', padding: '25px', marginBottom: '30px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '25px' }}>
            ...
            <div style={{ width: '100%', maxWidth: '600px', background: '#fdfdfd', padding: '20px', borderRadius: '16px', border: '1px solid #eee', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.02)' }}>
      ...
      <section className="card">
        ...
        <table>
          <thead>
            <tr>
              <th>Nama</th><th>NIM</th><th>Prodi</th><th>Semester</th><th>Tanggal</th><th>Keperluan</th><th>Action</th>
            </tr>
          </thead>
          <tbody>
            {waiting.length === 0 ? (
              ...
              waiting.map((app) => (
                <tr key={app.id}>
                  <td>{app.student_name}</td><td>{app.student_nim}</td><td>{app.student_prodi || '-'}</td><td>{app.student_semester || '-'}</td><td>{new Date(app.start_time).toLocaleString()}</td><td>{app.reason}</td>
                  <td>
      ...
      <section className="card">
        ...
        <form onSubmit={submitEvent}>
          <div className="form-group">
            ...
            <input type="text" value={eventForm.name} onChange={(e) => setEventForm({ ...eventForm, name: e.target.value })} required />
          </div>
          <div className="form-group">
            <label>Details:</label>
            <textarea value={eventForm.details} onChange={(e) => setEventForm({ ...eventForm, details: e.target.value })} required />
          </div>
      ...
      <section className="card">
        ...
        <table>
          <thead>
            <tr>
              <th>Nama</th><th>NIM</th><th>Alasan</th><th>Prodi</th><th>Semester</th><th>Jadwal Keluar</th><th>Waktu IN</th><th>Waktu OUT</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {permissions.length === 0 ? (
              <tr><td colSpan={9} style={{ textAlign: 'center' }}>Belum ada history perizinan.</td></tr>
            ) : (
              permissions.map((perm) => (
                <tr key={perm.id}>
                  <td>{perm.student_name || '-'}</td><td>{perm.student_nim || '-'}</td><td>{perm.reason}</td><td>{perm.student_prodi || '-'}</td><td>{perm.student_semester || '-'}</td><td>{new Date(perm.start_time).toLocaleString()}</td><td>{perm.check_in ? new Date(perm.check_in).toLocaleString() : '-'}</td><td>{perm.check_out ? new Date(perm.check_out).toLocaleString() : '-'}</td>
